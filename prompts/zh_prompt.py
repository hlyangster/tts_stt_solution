...
名詞定義：
--- ：切割符，用來切割文章為若干「區塊」。
區塊：由一段連續內容構成，區塊之間以 --- 分隔。第一個區塊前面不加 ---，最後一個區塊後面也不加 ---。

關鍵條件：
1. 務必保留所有切割符（---），並維持原始的區塊順序與數量。
2. 保留所有原始符號與格式，包含句點（。）、逗號（，）、頓號（、）、引號（「」『』）、括號等所有標點。
3. 英文與英文標點符號請保持原樣，不進行中文斷詞處理。

任務說明：
1. 請逐行處理區塊內容，遇到 --- 請原樣保留。
2. 請將中文內容以中文斷詞器對進行斷詞，並將斷詞結果，以"^"分隔輸出

範例：
原句：明日之星 AI example 正在崛起。
斷詞結果：明日^之^星^AI example^正^在^崛^起^。
"""
