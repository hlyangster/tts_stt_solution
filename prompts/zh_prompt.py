...
1.  **核心要求：保持原文結構與內容。** 原始文本中的斷句（以句號、問號、驚嘆號等結尾）、標點符號、換行，以及段落分隔符 "---" **必須** 被完整保留，不得有任何遺失或更改。這是最高優先級的指令。
2.  **處理單位定義：**
    *   **文本片段：** 指包含文字內容的句子或短語，通常以句號、問號、驚嘆號結尾，或者在換行符之前。
    *   **分隔符片段：** 指單獨一行且僅包含 "---" 的內容。
3.  **處理流程：**
    *   逐一識別文章中的每個「文本片段」和「分隔符片段」。
    *   **對於「文本片段」：**
        a.  **英文處理：** 對於片段中的英文單字或連續的英文片語（例如 "AI", "machine learning", "test case"），請將每個完整的英文單字（或由連字符連接的詞，如 "state-of-the-art"）視為一個**不可分割的單元**。不對英文單字內部進行斷詞。
        b.  **中文斷詞：** 在保留英文單元完整性的前提下，使用中文斷詞器對片段中的中文部分進行斷詞。
        c.  **分隔符應用：** 斷詞結果中的中文詞語之間、中文詞語與英文單元之間、以及相鄰的英文單元之間（如果原文中它們之間有空格的話），均使用 "^" 分隔。
        d.  **標點符號保留：** 該片段原有的所有標點符號（如逗號、句號等）**必須**保留在斷詞結果的正確位置，**不參與** "^" 分隔，而是作為斷詞結果的一部分。
    *   **對於「分隔符片段」（即單獨一行的 "---"）：**
        a.  **直接原樣輸出 "---"**。
        b.  **不進行斷詞**。
        c.  **不添加任何 "^" 符號**。
4.  **輸出格式：**
    *   按照原文順序，逐一輸出處理後的每個片段（斷詞後的文本片段或原始的 "---" 分隔符片段）。
    *   保持片段間原有的換行。
"""
